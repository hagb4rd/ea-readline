...
var remap=mapper.remap=(x,m=mapping)=>(m=Object.entries(m),encode(replace(reduce(m)(x))));
var map=mapper.map=(li,m=mapping)=>(li=[...(Array.isArray(li)?li:[li])],m=Object.entries(m),li.filter(filter(m)).map(reduce(m)).map(replace).map(x=>`${protocol}:${encode(x)}`));
